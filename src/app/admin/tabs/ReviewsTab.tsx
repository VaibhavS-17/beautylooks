...
import { Search, Star, Trash2, ExternalLink, MessageSquare, Loader2, Filter, Download } from 'lucide-react';
...
interface ReviewsTabProps {
  ...
  handleDeleteReview: (reviewId: string) => Promise<void> | void;
}
...
export default function ReviewsTab({
  ...
  const requestDeleteReview = (reviewId: string, customerName: string, productName: string) => {
    setConfirmModal({
      isOpen: true,
      title: 'Delete Customer Review',
      message: `Are you sure you want to permanently delete the review from "${customerName}" for "${productName}"?`,
      action: async () => {
  ...
  const handleExportCsv = () => {
    const headers = ['Review ID', 'Product Name', 'Customer Name', 'Rating', 'Comment', 'Created At'];
    ...
    exportToCsv('beautylooks_reviews', headers, rows);
  };
    ...
    <div className="space-y-8 animate-fade-in text-left">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#EFECE6] pb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Download size={14} />
            <span>Export CSV ({filteredReviews.length})</span>
          </button>
          ...
          {[5, 4, 3, 2, 1].map(stars => {
            ...
            return (
              <button
                key={stars}
                type="button"
                onClick={() => { setRatingFilter(ratingFilter === stars.toString() ? 'all' : stars.toString()); setCurrentPage(1); }}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {(searchQuery || ratingFilter !== 'all') && (
            <button
              onClick={() => { setSearchQuery(''); setRatingFilter('all'); setCurrentPage(1); }}
      ...
      {paginatedReviews.length === 0 ? (
        ...
        <>
            ...
            {paginatedReviews.map((review) => (
              ...
              >
                ...
                <div className="flex items-center justify-end pt-3 border-t border-[#EFECE6]/60">
                  <button
                    type="button"
                    onClick={() => requestDeleteReview(review.id, review.customerName, review.productName)}
          ...
          {/* Pagination Footer */}
          <div className="p-4 bg-[#FAF9F6] border border-[#EFECE6] rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
