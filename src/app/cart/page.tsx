...
import { Trash2, ShoppingBag, Plus, Minus, ArrowRight, ShieldCheck, Truck, Undo, AlertTriangle } from 'lucide-react';
...
export default function CartPage() {
  ...
  // Real-time stock map: productId -> current stock
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="space-y-3 sm:space-y-0 sm:divide-y sm:divide-border sm:bg-white sm:rounded-2xl sm:border sm:border-border sm:overflow-hidden sm:shadow-xs">
                {items.map((item) => {
                  ...
                  // Use real-time stock if available, otherwise fall back to cached
                  ...
                  return (
                    <div
                      key={item.product.id}
                      className={`p-4 sm:px-6 sm:py-3.5 bg-white sm:bg-transparent rounded-2xl sm:rounded-none border sm:border-0 shadow-xs sm:shadow-none hover:bg-white/60 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 ${
