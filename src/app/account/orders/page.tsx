      ...
      <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
          {(!orders || orders.length === 0) ? (
            ...
            orders.map((order) => (
              <Link key={order.id} href={`/account/orders/${order.id}`} className="block border border-[#EFECE6] rounded-xl p-5 bg-white space-y-4 shadow-sm hover:border-[#C9A94E] transition-colors cursor-pointer">
                {/* Order Header */}
                ...
                <div className="flex justify-between items-center border-t border-[#EFECE6] pt-3 text-sm">
                  ...
                  <span className="text-[#C9A94E] font-medium text-xs hover:underline">View Details →</span>
                </div>
              </Link>
            ))
          )}
